...
import static mpp.jatakamu.Constants.D_HOURS_IN_MILLS;
import static mpp.jatakamu.Constants.L_HOURS_IN_MILLS;
import static mpp.jatakamu.Constants.L_MINS_IN_MILLIS;
...
public class ViewUtils
{
    ...
    public static String toStringDegree(double degree)
    {
        long lDegree = Double.valueOf(
                Math.round(degree * D_HOURS_IN_MILLS))
                .longValue();
    ...
    public static String toStringDegreeForRaasi(double degree)
    {
        ...
        nDeg = nDeg % 30;
        long lDegree = Double.valueOf(
                Math.round((nDeg + degFracPart) * D_HOURS_IN_MILLS))
                .longValue();

        long deg = lDegree / L_HOURS_IN_MILLS;
        long min = (lDegree - (deg * L_HOURS_IN_MILLS))
                / L_MINS_IN_MILLIS;
        double sec = (lDegree - (deg * L_HOURS_IN_MILLS) - (min * L_MINS_IN_MILLIS)) / 1000D;
